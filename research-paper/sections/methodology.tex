...
\cs\ was built around four goals. \textbf{(G1) No egress:} a complete
audit must run with the network interface disabled. \textbf{(G2)
Multi-modal:} pattern analysis, structural metrics, semantic (LLM) review,
and dynamic execution should contribute to one result rather than living
in separate tools. \textbf{(G3) Context-preserving triage:} every finding
must be inspectable next to the exact source that produced it.
...
\label{sec:arch}
\input{figures/diagrams/fig-architecture}
...
\label{sec:pipeline}
\input{figures/diagrams/fig-pipeline}
...
\input{sections/tables/tab-modalities}
...
\label{sec:risk}
\input{figures/diagrams/fig-riskmodel}
